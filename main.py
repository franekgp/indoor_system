...
from models import Room, RoomCreate, CategoryUpdate
...
@app.get("/")
def test():
    return {"message": "Witoj chopie w systemie zarzadzania pokojami"}
...
@app.put("/rooms/{room_id}/category")
def update_category(room_id: int, category_update: CategoryUpdate, db_session: Session = Depends(get_db_session)):
    room = db_session.query(Room).filter(Room.id == room_id).first()
    if not room:
        raise HTTPException(status_code=404, detail="Room not found")
    
    room.room_category_id = category_update.room_category_id
    db_session.commit()
